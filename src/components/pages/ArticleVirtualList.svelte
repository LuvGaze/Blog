...
// 同分类文章恒定同一颜色（调色板取色），不同分类大概率明显不同
function getCategoryColor(name: string): string {
	return `--article-category-hue: ${getCategoryColorHue(name)}`;
}
...
function goToPinned(index: number) {
	...
	// 手动切换时重置自动轮播计时器：清除旧定时器并从当前索引重新开始倒计时
	stopPinnedCarousel();
	startPinnedCarousel();
	// 手动切换不触发封面「像素方块刷新」动画（该动画只在初次加载时发生）
}
	...
	{#if pinnedPosts.length > 0}
		...
		>
			<div class="article-list-pinned__heading">
				...
				<span class="article-list-pinned__count">共 <strong>{pinnedPosts.length}</strong> 篇置顶</span>
			</div>
			<div
			...
			>
				{#each pinnedPosts as pinnedPost, pinnedIndex (pinnedPost.id)}
					<article
						...
						style={`--article-category-hue: ${getCategoryColorHue(pinnedPost.category)}`}
					>
							<span class="article-list-pinned-item__badge">置顶</span>
					...
					<div class="article-list-masonry__col">
						{#each column as entry (entry.post.id)}
							...
							<article
								...
								style={`--article-category-hue: ${post.categoryHue ?? getCategoryColorHue(post.category)}`}
							>
								<div
		...
		<div class="article-list-view">
			...
			<div class="article-list-vertical article-list-vertical--pinned" aria-label="文章列表">
				{#each paginatedPosts as post (post.id)}
					<article class="article-list-pinned-item article-list-row-pinned" data-article-list-pinned-item style={`--article-category-hue: ${getCategoryColorHue(post.category)}`}>
						<div class="article-list-pinned-item__content">
							<h3 class="article-list-pinned-item__title">
