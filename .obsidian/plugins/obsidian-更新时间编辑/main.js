...
var UpdateTimeOnEditSettingsTab = class extends import_obsidian5.PluginSettingTab {
  ...
  createDateFormatEditor({
    ...
    const createDoc = () => {
      ...
      descr.append(
        ...
        descr.createEl("br"),
        `当前时间: ${format(new Date(), getValue())}`,
        descr.createEl("br"),
        `Obsidian 默认日期属性格式：yyyy-MM-dd'T'HH:mm`
      );
      return descr;
    };
  ...
  addEnableNumberProperties() {
    new import_obsidian5.Setting(this.containerEl).setName("启用数字属性类型").setDesc(
      "在使用数值格式（如 Unix 时间戳）时，将数字分配给日期属性（而不是字符串）。"
    ).addToggle(
  ...
  addTimeBetweenUpdates() {
    new import_obsidian5.Setting(this.containerEl).setName("更新之间的最小分钟数").setDesc("如果您的文件更新得太频繁，请增加这个。").addSlider(
  ...
  addEnableCreated() {
    new import_obsidian5.Setting(this.containerEl).setName("启用创建的前置事项键更新").setDesc("目前，如果没有设置，默认为现在Now").addToggle(
  ...
  addFrontMatterUpdated() {
    new import_obsidian5.Setting(this.containerEl).setName("前言更新名称").setDesc("更新前言YAML中的关键字的时间。").addText(
      (text) => {
        var _a;
  ...
  addFrontMatterCreated() {
    ...
    new import_obsidian5.Setting(this.containerEl).setName("前言创建名称").setDesc("前言 YAML 中用于创建时间的功能").addText(
      (text) => {
        var _a;
  ...
  addExcludedCreatedFoldersSetting() {
    ...
    this.doSearchAndRemoveList({
      ...
      name: "排除更新已创建属性的文件夹",
      description: "在此文件夹中更新的任何文件都不会触发创建更新。"
    });
  }
  addExcludedFoldersSetting() {
    this.doSearchAndRemoveList({
      ...
      name: "无需更新的文件夹",
      description: "在此文件夹中更新的任何文件将不会触发已更新和已创建的更新。"
    });
  }
  doSearchAndRemoveList({
    ...
    currentList.forEach(
      (ignoreFolder) => new import_obsidian5.Setting(this.containerEl).setName(ignoreFolder).addButton(
        (button) => button.setButtonText("移除").onClick(async () => {
